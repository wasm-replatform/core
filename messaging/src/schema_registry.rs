...
use crate::{MessagingError, SchemaConfig};
...
impl DecodedPayload<'_> {
    ...
    pub fn decode(buffer: &[u8]) -> Result<DecodedPayload<'_>, MessagingError> {
        ...
        Ok(DecodedPayload { magic_byte, registry_id, payload })
    }
}
...
impl SRClient {
    pub fn new(schema_cfg: SchemaConfig) -> Self {
        ...
        let sr_client = Self { client, schemas: Arc::clone(&schemas) };
    ...
    pub async fn validate_and_encode_json(
        &self, topic: &str, buffer: Vec<u8>,
    ) -> Result<Vec<u8>, MessagingError> {
        // If schema registry is available, use it
        if self.client.is_some() {
            let (id, schema) = self.get_or_fetch_schema(topic).await?;
            let payload: Value = serde_json::from_slice(&buffer).map_err(|e| {
                MessagingError::SchemaRegistryError(format!("Invalid JSON: {:?}", e))
            })?;
    ...
    pub async fn validate_and_decode_json(
        ...
        if self.client.is_some() {
            ...
            let payload: Value = serde_json::from_slice(&message.payload).map_err(|e| {
                MessagingError::SchemaRegistryError(format!("Invalid JSON: {:?}", e))
            })?;
    ...
    async fn get_or_fetch_schema(&self, topic: &str) -> Result<(u32, Value), MessagingError> {
        ...
        if let Some((id, value)) = schemas.get(topic) {
            ...
            let schema_response = sr.get_latest_version(&subject, None).await.map_err(|e| {
            ...
            let schema_json: Value = serde_json::from_str(schema_str).map_err(|e| {
                MessagingError::SchemaRegistryError(format!("Invalid schema JSON: {:?}", e))
            })?;

            let registry_id = schema_response.id.ok_or_else(|| {
                MessagingError::SchemaRegistryError(format!(
                    "Registry ID for topic {} is missing",
                    topic
                ))
            })? as u32;
